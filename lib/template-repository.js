    ...
    async.auto({
      template: (done) => {
        if (this.cache[template]) {
          return done(null, this.cache[template]);
        }
      ...
      globals: (done) => {
        if (this.cache.global) {
          return done(null, this.cache.global);
        }
      ...
      subject: ['template', 'globals', 'html', (done, results) => {
        if (data.subject) {
          done(null, this.renderer.renderText(data.subject, data));
        } else if (results.template.details.renderSubject) {
